...
app = FastAPI(title="Obs + IA (bypass v2)", version="1.4.0")
...
def build_summary(index: str, time_range: str, service: Optional[str], size: int) -> Dict[str, Any]:
    ...
    if service: must_filters.append({"term":{"service.name":service}})
    ...
    return {"total": resp.get("hits",{}).get("total",{}).get("value",0),
...
def incident_summary(req: SummaryRequest):
    ...
    if req.service: question += f" focado no serviço {req.service}"
